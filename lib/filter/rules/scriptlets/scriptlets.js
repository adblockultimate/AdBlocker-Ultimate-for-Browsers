    ...
    function adjustSetInterval(source, matchCallback, matchDelay, boost) {
      ...
      var intervalWrapper = function intervalWrapper(callback, delay) {
        if (!isValidCallback(callback)) {
          var message = `Scriptlet can't be applied because of invalid callback: '${String(
            callback
          )}'`;
          logMessage(source, message);
        } else if (
      ...
      var extractNewHref = function extractNewHref(anchor, attr) {
        ...
        if (attr.startsWith('?')) {
          try {
            ...
            logMessage(
              source,
              `Cannot retrieve the parameter '${attr.slice(1)}' from the URL '${
                anchor.href
              }`
            );
            return '';
    ...
    function jsonPruneFetchResponse(source, propsToRemove, obligatoryProps) {
      ...
      var fetchHandlerWrapper = async function fetchHandlerWrapper(
        ...
        try {
          ...
          var message = `Response body can't be converted to json: ${objectToString(
            fetchData
          )}`;
          logMessage(source, message);
    ...
    function logAddEventListener(source) {
      ...
      function addEventListenerWrapper(type, listener) {
        ...
        if (validateType(type) && validateListener(listener)) {
          var message = `addEventListener("${type}", ${listenerToString(
            listener
          )})`;
  ...
  function nowebrtc(source, args) {
    ...
    function nowebrtc(source) {
      ...
      var rtcReplacement = function rtcReplacement(config) {
        var message = `Document tried to create an RTCPeerConnection: ${convertRtcConfigToString(
          config
        )}`;
        logMessage(source, message);
        hit(source);
  ...
  function removeNodeText(source, args) {
    ...
    function handleExistingNodes(selector, handler, parentSelector) {
      var processNodes = function processNodes(parent) {
        if (selector === '#text') {
          var textNodes = nodeListToArray(parent.childNodes).filter(function (
            node
          ) {
  ...
  function spoofCSS(source, args) {
    ...
    function spoofCSS(source, selectors, cssPropertyName, cssPropertyValue) {
      ...
      function convertToCamelCase(cssProperty) {
        ...
        return `${firstPart}${secondPart[0].toUpperCase()}${secondPart.slice(
          1
        )}`;
      }
  ...
  function trustedClickElement(source, args) {
    ...
    function trustedClickElement(source, selectors) {
      ...
      if (cookieMatches.length > 0) {
        ...
        var cookiesMatched = Object.keys(parsedCookieMatches).every(function (
          key
        ) {
      ...
      var findElements = function findElements(mutations, observer) {
        ...
        var allSelectorsFulfilled = selectorsSequence.every(function (
          selector
        ) {
